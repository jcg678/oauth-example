...
class LinkedinController extends Controller
{
    ...
    public function connectCheckAction(Request $request)
    {
        ...
        $userIN = $this->getDoctrine()->getManager()->getRepository('AppBundle:User')->findOneBy(['googleID' => $id]);
        ...
        if ($check == 0) {
            ...
            $userlast->setGoogleID($user->getId());
            $em->persist($userlast);
            $em->flush();
        ...
        return $this->render('linkedin/linkedin.html.twig', [
            'user' => $user,
            'check' => $check
        ]);
